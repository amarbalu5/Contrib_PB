...
try:
    ...
    st.warning("Install 'kaleido' and 'reportlab' packages for PDF export functionality")
...
def export_plots_to_single_pdf(figures: List[go.Figure]) -> bytes:
    """Export multiple plots to a single PDF with each plot on a separate page"""
    if not PDF_EXPORT_AVAILABLE:
        st.error("PDF export not available. Please install 'kaleido' and 'reportlab' packages.")
        return b""
    
    try:
        # Create a BytesIO buffer for the combined PDF
        pdf_buffer = io.BytesIO()
        
        # Convert each figure to PNG first (better quality than direct PDF)
        plot_images = []
        for fig in figures:
            img_bytes = fig.to_image(format="png", width=800, height=600, engine="kaleido")
            plot_images.append(img_bytes)
        ...
        for i, img_bytes in enumerate(plot_images):
            if i > 0:  # Add new page for each plot except the first
                c.showPage()
            ...
            img_width = 800  # Original width
            img_height = 600  # Original height
            ...
            # Draw the image
            c.drawImage(img, x, y, width=final_width, height=final_height)
